...
class ReferenceExtractor:
    ...
    def extract(self, normalized_body):
        ...
        for pattern in explicit_ref_patterns:
            ...
            if match:
                ...
                if self._is_plausible_reference(ref):
        ...
        for pattern in known_patterns:
            ...
            for match in matches:
                if (self._is_plausible_reference(match) and
                    not match.isdigit() and
                    not re.match(r'\d{16}', match) and  # Exclure numéros de carte
                    not re.match(r'\d{2}/\d{2}/\d{4}', match) and  # Exclure dates
                    not re.match(r'\d{2}:\d{2}', match) and  # Exclure heures
                    not match.upper() in ['MASTERCARD', 'VISA', 'CARTE', 'DEBIT', 'CREDIT']):
                    cleaned_ref = self._clean_reference(match)
                    return cleaned_ref
        ...
        for pattern in ref_contexts:
            ...
            if match:
                ...
                if self._is_plausible_reference(potential_ref):
                    cleaned_ref = self._clean_reference(potential_ref)
                    return cleaned_ref
        ...
        if "ABH" in normalized_body:
            ...
            if abh_match:
                ...
                if self._is_plausible_reference(code):
                    return code
    ...
    def _is_plausible_reference(self, ref):
        """Détermine si une chaîne ressemble à une référence valide - VERSION AMÉLIORÉE"""
        ...
        # LISTE ÉTENDUE DES TERMES À EXCLURE
        excluded_terms = [
        ...
        if re.match(r'^\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}$', ref):  # Dates
            return False
        if re.match(r'^\d{1,2}:\d{2}$', ref):  # Heures
            return False
        if re.match(r'^TX_\d+$', ref):  # Anciens formats de date que nous générions
            return False
